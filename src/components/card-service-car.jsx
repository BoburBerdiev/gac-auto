...
const CardServiceCar = ({isIndexPage, text, icon, extraStyleIcon, download, href , id, onClick}) => {
    return (
        ...
        <motion.a key={`${id}Child`} whileTap={{scale: 0.9, y: 10}} onClick={onClick} download={download} href={href}  className={`flex justify-center ${isIndexPage ? 'flex-col' : 'flex-row md:flex-col'} cursor-pointer items-center  gap-2 md:gap-3 p-4 md:p-5`}>
            {
